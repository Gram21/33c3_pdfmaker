...
class PdfMaker:
  ...
  def flag(self, cmd):
    return "Find the flag here: watch?v=HF4hJ_2aNg8"
  ...
  def compilePDF(self, cmd):
    ...
    return timeout(self.startCompilationProcess, args=(full_filename,))


  def startCompilationProcess(self, full_filename):
    compile_command = "cd " + self.directory + " && pdflatex -interaction=batchmode " + pipes.quote(full_filename)
    try:
      res = subprocess.check_output(compile_command, shell=True)
    except subprocess.CalledProcessError as exc:
      res = "Compilation exited with status " + str(exc.returncode)
    return res
...
def timeout(func, args=(), kwargs={}, timeout_duration=24, default="Compilation timed out. If this persists your tex file is insufficient."):
  import signal
